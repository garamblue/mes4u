<template>
  <div class="components-container">
          ...
          <el-form ref="packing" :model="operationForm" label-width="180px" class="form-container">
            <el-col :span="18">
              <el-form-item id="operationname" label="Operation Name">
                <Searchselect v-model="operationForm.optype" placeholder="Operation Name" name="operation" @keyup.enter="findForm" />
              </el-form-item>
              <el-form-item id="operationdescription" label="Operation Description">
                <el-input v-model="operationForm.opdesc" placeholder="Operation Description" @keyup.enter="findForm" />
              </el-form-item>
              <el-form-item id="operationpartitem" label="Item Number">
                <el-col :span="10">
                  <el-input v-model="operationForm.itemNumber" placeholder="Item Number" @blur="setItem" @keyup.enter="findForm">
                    <template #prepend>
                      <el-button icon="Search" @click="handleItemModal" />
                    </template>
                  </el-input>
                </el-col>
                <el-col :span="14">
            ...
            >
              <el-table-column label="Operation" width="150">
                <template #default="{row}">
                  ...
                  <span v-else><el-input v-model="row.optype" @keyup.enter="modifyOpData(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
              <el-table-column label="Description">
                <template #default="{row}">
                  <span v-if="!row.edit">{{ row.opdesc }}</span>
                  <span v-else><el-input v-model="row.opdesc" @keyup.enter="modifyOpData(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
          ...
          >
            <el-table-column label="Item Number" align="left" min-width="120" fixed>
              <template #default="{row}">
                ...
                <span v-else><el-input v-model="row.item_number" @keyup.esc="row.edit=!row.edit" @keyup.enter="modifyPartData(row)" @blur="changeItemId(row)" /></span>
              </template>
            </el-table-column>
            <el-table-column label="Barcode Type" align="left" min-width="80">
              <template #default="{row}">
                <span v-if="!row.edit">{{ row.barcode_type }}</span>
                <span v-else><el-input v-model="row.barcode_type" @keyup.enter="modifyPartData(row)" /></span>
              </template>
            </el-table-column>
            <el-table-column label="Indicator" align="left">
              <template #default="{row}">
                <span v-if="!row.edit">{{ row.indicator }}</span>
                <span v-else><el-input v-model="row.indicator" @keyup.esc="row.edit=!row.edit" @keyup.enter="modifyPartData(row)" /></span>
              </template>
            </el-table-column>
            <el-table-column label="Location" align="left" min-width="140">
              <template #default="{row}">
                <span v-if="!row.edit">{{ row.location }}</span>
                <span v-else><el-input v-model="row.location" size="small" @keyup.esc="row.edit=!row.edit" @keyup.enter="modifyPartData(row)" /></span>
              </template>
            </el-table-column>
            <el-table-column label="Length" align="left" min-width="140">
              <template #default="{row}">
                <span v-if="!row.edit">{{ row.length }}</span>
                <span v-else><el-input-number v-model="row.length" size="small" @keyup.esc="row.edit=!row.edit" @keyup.enter="modifyPartData(row)" /></span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="Seperator" align="left" min-width="140">
              <template #default="{row}">
                ...
                <span v-else><el-input-number v-model="row.seperator" size="small" @keyup.esc="row.edit=!row.edit" @keyup.enter="modifyPartData(row)" /></span>
              </template>
            </el-table-column>
    ...
    <LOVItem :dialog-visible="itemDialogVisible" :input-item="operationForm.itemNumber" @apply-item="selectItem" />

    <el-dialog v-model="operationDialogVisible" title="Create Operation" @before-close="operationDialogVisible=false">
    ...
    <el-dialog v-model="opPartsDialogVisible" title="Create Operation Parts" @before-close="opPartsDialogVisible=false">
      <el-row>
        <el-col :lg="24">
