<template>
  <div class="components-container">
      ...
      <el-row>
        ...
        <el-card class="box-card" style="margin-bottom:10px;">
          <el-row :gutter="10">
            <el-form ref="dataForm" :inline="true" :model="findForm" class="demo-form-inline" label-position="top">
              ...
              <el-form-item id="searchcode" label="Defect Code">
                <el-input v-model="findForm.code" placeholder="Code" @keyup.enter="onFind" />
              </el-form-item>
          ...
          <el-row>
            ...
            >
              ...
              <el-table-column label="NAME" align="center">
                <template #default="{row}">
                  ...
                  <span v-else><el-input v-model="row.name" @keyup.enter="modifyCode(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
              <el-table-column label="ALARM COUNT" align="center">
                <template #default="{row}">
                  <span v-if="!row.edit">{{ row.alarmCount }}</span>
                  <span v-else><el-input v-model="row.alarmCount" @keyup.enter="modifyCode(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
              <el-table-column label="LV1 CODE" align="center">
                <template #default="{row}">
                  <span v-if="!row.edit">{{ row.parentCodeLv1 }}</span>
                  <span v-else><el-input v-model="row.parentCodeLv1" @keyup.enter="modifyCode(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
              <el-table-column label="LV2 CODE" align="center" class-name="small-padding fixed-width">
                <template #default="{row}">
                  ...
                  <span v-else><el-input v-model="row.parentCodeLv2" @keyup.enter="modifyCode(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
              <el-table-column label="LV3 CODE">
                <template #default="{row}">
                  <span v-if="!row.edit">{{ row.parentCodeLv3 }}</span>
                  <span v-else><el-input v-model="row.parentCodeLv3" @keyup.enter="modifyCode(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
              <el-table-column label="LV4 CODE">
                <template #default="{row}">
                  <span v-if="!row.edit">{{ row.parentCodeLv4 }}</span>
                  <span v-else><el-input v-model="row.parentCodeLv4" @keyup.enter="modifyCode(row)" @keyup.esc="row.edit=!row.edit" /></span>
                </template>
              </el-table-column>
            ...
            <pagination v-show="total>0" v-model:page="listQuery.page" v-model:limit="listQuery.limit" :total="total" @pagination="onFind" />
    ...
    <el-dialog v-model="createDialogVisible" title="Create Code">
