    ...
    <el-col>
      <div class="components-container">
        ...
        <div class="mixin-components-container">
                    ...
                    <el-form label-position="top" :inline="true" :model="listQuery">
                      <el-col :xs="24" :sm="24" :md="24" :lg="24">
                        <el-form-item id="searchitem" label="Item">
                          <el-input v-model="listQuery.item" placeholder="Item" class="filter-item" @keyup.enter="handleFilter" /> <!-- Text -->
                        </el-form-item>
                        ...
                        <el-form-item id="searchjobid" label="ID">
                          <el-input v-model="listQuery.id" placeholder="ID" class="filter-item" @keyup.enter="handleFilter" />
                        </el-form-item>
                ...
                >
                  ...
                  <el-table-column label="Line" align="center" min-width="90" fixed>
                    <template #default="{row}">
                      ...
                      <span v-else><el-input v-model="row.line" autofocus @keyup.enter="updateJob(row)" @keyup.esc="row.edit=!row.edit" /></span>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column label="Details" align="center" min-width="300" class-name="small-padding fixed-width" fixed="right">
                    <template #default="{row}">
                      <Btn name="serial" icon="DocumentCopy" @click="getSerialList(row)" />
                      <Btn name="operation" icon="Tickets" @click="getOperationList(row)" />
                    </template>
                  </el-table-column>
                </el-table>

                <pagination v-show="total>0" v-model:page="listQuery.page" v-model:limit="listQuery.limit" :total="total" @pagination="getList" />
          ...
          <el-dialog v-model="dialogFormVisible" :title="title" :before-close="closeDialog" width="90%">
              ...
              >
                <el-table-column label="Seq" min-width="80" fixed>
                  <template #default="{row}">
                    ...
                    <span v-else><el-input v-model="row.seq" @keyup.enter="updateOperation(row)" /></span>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="Type" min-width="60">
                  <template #default="{row}">
                    ...
                    <span v-else><el-input v-model="row.barcodetype" @keyup.enter="updateOperation(row)" /></span>
                  </template>
                </el-table-column>
                <el-table-column label="Indicator" min-width="120">
                  <template #default="{row}">
                    <span v-if="!row.edit">{{ row.indicator }}</span>
                    <span v-else><el-input v-model="row.indicator" @keyup.enter="updateOperation(row)" /></span>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="Location" min-width="80">
                  <template #default="{row}">
                    ...
                    <span v-else><el-input v-model="row.location" @keyup.enter="updateOperation(row)" /></span>
                  </template>
                </el-table-column>
                <el-table-column label="Length" min-width="80">
                  <template #default="{row}">
                    <span v-if="!row.edit">{{ row.length }}</span>
                    <span v-else><el-input v-model="row.length" @keyup.enter="updateOperation(row)" /></span>
                  </template>
                </el-table-column>
            ...
            <template v-if="dialogStatus === 'create'">
              <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="150px" style="margin-left:50px;">
                ...
                <el-form-item label="Quantity" prop="qty">
                  <el-input-number v-model="temp.qty" @keyup.enter="createJob()" />
                </el-form-item>
              </el-form>
            </template>
            <template v-if="dialogStatus === 'serial'" #footer>
              <div class="dialog-footer">
                <Btn name="close" @click="closeDialog" />
              </div>
            </template>
            <template v-else-if="dialogStatus === 'operation'" #footer>
              <div class="dialog-footer">
                <Btn name="close" @click="closeDialog" />
              </div>
            </template>
            <template v-else-if="dialogStatus === 'create'" #footer>
              <div class="dialog-footer">
