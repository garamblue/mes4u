<template>
  <el-row>
    <el-col>
              ...
              <div id="status">
                <el-row>
                  <el-col :span="12">
                    ...
                    <Btn id="reset" name="reset" icon="Setting" @click="resetStatus" />
                  </el-col>
                </el-row>
              </div>
            </el-card>
                ...
                <el-row>
                  <el-col :span="12">
                    <el-input
                      ...
                      autofocus
                      @keyup.enter="handleAssyBarcodeScan"
                    />
                  </el-col>
                </el-row>
              </div>
                        ...
                        <template #default="{row,$index}">
                          <template v-if="row.edit">
                            <el-input
                              ...
                              class="edit-input"
                              size="default"
                              @keyup.enter="handlePartScan(row, $index)"
                            />
                          </template>
            ...
            <el-card id="history" class="box-card" style="margin-bottom:10px;">
              <template #header>
                <div class="clearfix">
                  <strong>
                    <span>
                      <el-icon><Reading /></el-icon> Operation Histories
                    </span>
                  </strong>
                  <historytable :data="historyData" :list-loading="listLoading" :cols="cols" @sort-change="sortChange" />
                  <pagination
                    v-show="total > 0"
      ...
      <!-- Modal -->
      <el-dialog v-model="dialogFormVisible" title="Operation Setting" width="90%">
...
export default {
  ...
  methods: {
    ...
    async setOperation() {
      const selection = this.$refs['settingtable'].getSelectionRows()
      var idx = 1
    ...
    nextPart(row) {
      ...
      this.$refs['parttable'].getSelectionRows().forEach((e, i) => {
        if (nextflag === true) {
          nextrow = e
          nextflag = false
        }
